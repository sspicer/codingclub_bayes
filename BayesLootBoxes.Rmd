...
## Difference in Y/N for gateway question...
...
wilcox.test(PGSI ~ LBContribGamble1, data = LBfirst)
ttestBF(formula = PGSI ~ LBContribGamble1, data = LBfirst)


```
```{r}

LBfirst %>% anova_test(LootBoxSpend ~ Gender*LBContribGamble1, type = 3)

LBfirst$LBContribGamble1 <- as.factor(LBfirst$LBContribGamble1)
LBfirst$Gender <- as.factor(LBfirst$Gender)
LBfirst$PID <- as.factor(LBfirst$PID)


bf <- anovaBF(formula = LootBoxSpend ~ Gender*LBContribGamble1, 
              data = LBfirst)
bf
...
## Additional analysis - separate to analysing gateway effects.
...
VarSpendKendall <- as.numeric(cor_out$estimate)
...
Regression modelling... import data.
...
bayesreg <- brm(bf(RLI ~ IGD+GRCS), 
                      ...
                      data = regdata_z, 
                      #family = gaussian(),
                      #ModelType <- set_prior("normal(0,2)", class = "b"),
                      file = "minimulti.rds")
